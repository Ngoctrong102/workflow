...
import { Plus, Trash2, Edit2, ChevronDown, ChevronRight, Eye, Code } from "lucide-react"
import { cn } from "@/lib/utils"
import type { FieldDefinition, SchemaDefinition } from "./SchemaEditor"
import { MvelExpressionEditor } from "@/components/workflow/MvelExpressionEditor"
...
interface OutputSchemaEditorProps {
  ...
  onMappingChange: (outputMapping: Record<string, string>) => void
  nodes?: Node[]
...
/**
 * Extended FieldDefinition for Output Schema with expression mapping
 */
interface OutputFieldDefinition extends FieldDefinition {
  expression?: string // MVEL expression to extract value from response
}
...
export function OutputSchemaEditor({
  ...
  const handleAddField = (schemaIndex: number, parentFieldIndex: number | null = null) => {
    ...
    if (parentFieldIndex === null) {
      ...
      onChange(newSchemas)
      setEditingFieldIndex({ schemaIndex, fieldIndex: newSchemas[schemaIndex].fields.length - 1 })
    } else {
      ...
      if (parentField && (parentField.type === 'object' || parentField.type === 'array')) {
        ...
        onChange(newSchemas)
        setEditingFieldIndex({ schemaIndex, fieldIndex: parentField.fields.length - 1 })
      }
    }
  }
  ...
  // Get field path for mapping (recursive)
  const getFieldPath = (
    schemaIndex: number,
    fieldIndex: number | null,
    parentFieldPath: string[] = []
  ): string => {
    if (fieldIndex === null || !schemas[schemaIndex]) return ""
    
    let currentFields = schemas[schemaIndex].fields
    
    // Navigate through parent path to get to nested fields
    for (const parentFieldName of parentFieldPath) {
      const parentField = currentFields.find(f => f.name === parentFieldName)
    ...
    // Build path: parentPath.fieldName
    return parentFieldPath.length > 0 
      ? [...parentFieldPath, field.name].join(".")
      : field.name
  }

  const handleExpressionChange = (fieldPath: string, expression: string) => {
    onMappingChange({
      ...outputMapping,
  ...
  const renderFieldEditor = (
    ...
    const fieldPath = getFieldPath(schemaIndex, fieldIndex, parentFieldPath)
    const currentExpression = fieldPath ? (outputMapping[fieldPath] || "") : ""
              ...
              {fieldPath && (
                <div className="space-y-1.5 pt-2 border-t">
                  <div className="flex items-center gap-2">
                    ...
                    <Label htmlFor={`field-expression-${schemaIndex}-${fieldIndex}`} className="text-xs font-semibold">
                      MVEL Expression (Output Mapping)
                    </Label>
                  </div>
                  <MvelExpressionEditor
                    value={currentExpression}
                    onChange={(value) => handleExpressionChange(fieldPath, value)}
                    field={field}
                    nodes={nodes}
                    currentNodeId={currentNodeId}
                    triggerSchema={triggerSchema}
                    placeholder={`@{_response.${field.name || 'field'}}`}
                    label=""
                    description="MVEL expression to extract this field's value from action response"
                  />
                </div>
              )}
                  ...
                  {field.fields && field.fields.length > 0 && (
                    <div className="space-y-2">
                      {field.fields.map((nestedField, nestedIndex) => (
                        <div key={nestedIndex}>
                          {renderFieldEditor(nestedField, schemaIndex, nestedIndex, fieldIndex, [...parentFieldPath, field.name], level + 1)}
                        </div>
                      ))}
                    </div>
                  )}
    ...
    return (
      ...
      >
        <div className="flex items-center space-x-2 flex-1 min-w-0">
          ...
          {fieldPath && outputMapping[fieldPath] && (
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-secondary-500 mt-1">
            Define output fields with MVEL expressions to map from action response
          </p>
        </div>
        <div className="flex items-center space-x-2">
          <Button
